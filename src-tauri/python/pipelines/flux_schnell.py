...
def generate(
    ...
    progress_callback({"progress": 0.2, "message": "Loading Flux Schnell", "phase": "load"})
    pipe = load_diffusion_pipeline(model_dir, hf_repo, FluxPipeline, torch.bfloat16)
    ...
    result = pipe(
        ...
        generator=generator,
        guidance_scale=float(params.get("cfg", 0.0)),
    ...
    out = make_output_path(output_dir, "flux-schnell", "png")
    image.save(out)
    return {
        "file_path": str(out),
        "media_type": "image",
