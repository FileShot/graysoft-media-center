...
def generate(
    ...
    progress_callback({"progress": 0.2, "message": "Loading Hunyuan Video pipeline", "phase": "load"})
    try:
        from diffusers import HunyuanVideoPipeline
    except ImportError as exc:
        raise RuntimeError(
            "Hunyuan Video requires a recent diffusers build with HunyuanVideoPipeline support"
        ) from exc

    pipe = load_diffusion_pipeline(model_dir, hf_repo, HunyuanVideoPipeline, torch.bfloat16)
    ...
    out = make_output_path(output_dir, "hunyuan-video", "mp4")
